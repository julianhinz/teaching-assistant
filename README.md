...
**Model Option:** The `--model` option allows you to specify which model to use. By default, it uses `anthropic/claude-4.5-opus` via OpenRouter. For OpenRouter models (format: `provider/model-name` with known providers like `anthropic`, `openai`, `google`, `meta-llama`, `mistralai`, etc.), set the `OPENROUTER_API_KEY` environment variable. For direct Anthropic API models (e.g., `claude-3-5-sonnet-20241022`), set the `ANTHROPIC_API_KEY` environment variable.
